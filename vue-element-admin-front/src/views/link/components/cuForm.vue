<template>
  <el-dialog :visible.sync="dialogVisible" :title="curId ? '编辑链接' : '新增链接列表'" width="700px" :before-close="close">
    <el-form ref="ruleForm" label-position="left  " :model="ruleForm" status-icon :rules="rules" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item v-show="curId" label="链接ID">
...
<script>
import { getById, addLink, updateLink } from '@/api/link'
export default {
  name: 'CuForm',
  props: {
    dialogVisible: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.curId) {
            ...
            updateLink(this.curId, this.ruleForm).then(res => {
              this.$message({
                message: '修改成功',
                type: 'success'
              })
            ...
            addLink(this.ruleForm).then(res => {
              this.$message({
                message: '新增成功',
                type: 'success'
              })
