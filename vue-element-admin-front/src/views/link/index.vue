...
import { deleteLink, deleteLinks, getLinks } from '@/api/link'
export default {
  name: 'Roles',
  components: { cuForm },
  data() {
  ...
  methods: {
    ...
    deleteLinks() {
      this.$confirm('此操作将从链接列表中中移除选中链接' + ', 是否继续？', '提示', {
        ...
        deleteLinks(this.multipleSelection).then(res => {
          this.$message({
            message: '删除成功',
            type: 'success'
          })
